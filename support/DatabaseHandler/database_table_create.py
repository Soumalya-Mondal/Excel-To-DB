...
def database_table_create(env_file_path: str, table_name: str, table_create_sql: str)  -> dict[str, str]: #type: ignore
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S01] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S02] - {str(error)}'}

    # load ".env" file"S03
    try:
        load_dotenv(dotenv_path = env_file_path_object)
    except Exception as error:
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S03] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S04] - {str(error)}'}

    # define "table_name" table check sql query:S05
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S05] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S06] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S07] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[Database-Table-Create:S08] - {str(error)}'}