...
def database_table_create(env_file_path: str, table_name: str, table_create_sql: str)  -> dict[str, str]: #type: ignore
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S01] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S02] - {str(error)}'}

    # load ".env" file"S03
    try:
        load_dotenv(dotenv_path = env_file_path_object)
    except Exception as error:
        return {'status' : 'ERROR', 'message': f'[S03] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S04] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S05] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S06] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S07] - {str(error)}'}
    ...
    try:
        ...
        return {'status' : 'ERROR', 'message': f'[S08] - {str(error)}'}